...
Este proyecto ya incluye `vercel.json` para que Vercel detecte correctamente Next.js.

1. Crea tus variables de entorno de producción en Vercel (Project Settings -> Environment Variables), al menos:

```
BACKEND_URL=https://tu-backend.ejemplo.com
```

Puedes usar `/.env.example` como referencia para variables opcionales.

2. Importa el repositorio en Vercel y define el **Root Directory** como:

```
girona-front
```

3. Vercel usará automáticamente:
- Install command: `npm install`
- Build command: `npm run build`

4. Opcional por CLI (desde `girona-front`):
