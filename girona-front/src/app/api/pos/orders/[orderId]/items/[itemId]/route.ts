...
export async function DELETE(
  ...
  if (!orderId || !itemId) {
    return NextResponse.json({ message: "ID de orden e item requeridos" }, { status: 400 });
  }

  const backendBaseUrl = getBackendBaseUrl();
  const url = toAbsoluteUrl(backendBaseUrl, `/pos/orders/${orderId}/items/${itemId}`);
  ...
  try {
    ...
    return NextResponse.json(
      {
        message:
          "No se pudo conectar con el backend para eliminar el item de la orden. Verifica que Uvicorn esté corriendo y que `BACKEND_URL` sea accesible desde el servidor de Next.js.",
