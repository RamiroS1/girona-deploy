...
export async function GET() {
  ...
  const url = toAbsoluteUrl(backendBaseUrl, "/pos/orders");
  ...
  try {
    ...
    return NextResponse.json(
      {
        message:
          "No se pudo conectar con el backend para cargar las órdenes. Verifica que Uvicorn esté corriendo y que `BACKEND_URL` sea accesible desde el servidor de Next.js.",
...
export async function POST(request: Request) {
  ...
  if (!table_id) {
    return NextResponse.json({ message: "Mesa requerida" }, { status: 400 });
  }
  ...
  try {
    ...
    return NextResponse.json(
      {
        message:
          "No se pudo conectar con el backend para crear la orden. Verifica que Uvicorn esté corriendo y que `BACKEND_URL` sea accesible desde el servidor de Next.js.",
        backendUrl: backendBaseUrl,
        triedUrl: url,
        error: errorToJson(error),
      },
