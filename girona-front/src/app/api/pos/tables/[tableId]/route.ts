...
export async function DELETE(
  ...
  if (!tableId) {
    return NextResponse.json({ message: "ID de mesa requerido" }, { status: 400 });
  }

  const backendBaseUrl = getBackendBaseUrl();
  const url = toAbsoluteUrl(backendBaseUrl, `/pos/tables/${tableId}`);
  ...
  try {
    ...
    return NextResponse.json(
      {
        message:
          "No se pudo conectar con el backend para eliminar la mesa. Verifica que Uvicorn esté corriendo y que `BACKEND_URL` sea accesible desde el servidor de Next.js.",
