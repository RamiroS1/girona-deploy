...
export async function GET(request: Request) {
  ...
  const baseUrl = toAbsoluteUrl(backendBaseUrl, "/sales/summary/tables");
  ...
  try {
    ...
    return NextResponse.json(
      {
        message:
          "No se pudo conectar con el backend para cargar ventas por mesa. Verifica que Uvicorn esté corriendo y que `BACKEND_URL` sea accesible desde el servidor de Next.js.",
  ...
  if (!response.ok) {
    const message =
      ...
      "No se pudieron cargar las ventas por mesa";
